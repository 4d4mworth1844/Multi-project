...
async function subcribeAutomation() {
    const clientId = `mqtt_${Math.random().toString(16).slice(3)}`;                                 // client ID
    const connectUrl = `mqtt://${process.env.MQTT_BROKER_HOST}:${process.env.MQTT_BROKER_PORT}`;    // mqtt broker url
    const topic = process.env.AUTOMATION_CHANNEL;                                                         // topic to subcribe
    // Create a mqtt connection object and configuration of itself
    ...
    client.on('message', async (topic, payload) => {
        ...
        if (dataMessage === "request") {
            ...
            for (var i = 0; i < len; i++) {
                if (feeds[i]['type'] === "button" || feeds[i]['type'] == "task") {
                    var feedStatus = [];
                    feedStatus.push(feeds[i]["feedKey"]); 
                    feedStatus.push(feeds[i]["automated"]);
    
                    publishAutomation(feedStatus); 
                    await sleep(200); 
                }
            }
        } else {
    ...
    client.on('message', async (topic, payload) => {
       ...
       // console.log("Success");
        if (dataMessage === "request") {
            ...
            var len = feeds.length; 
            for (var i = 0; i < len; i++) {
                if (feeds[i]['type'] === "button") {
                    var feedStatus = [];
                    feedStatus.push(feeds[i]["feedKey"]); 
                    feedStatus.push(feeds[i]["status"]);
    
                    publishFeed(feedStatus); 
                    await sleep(200); 
                }
            }
        } else {
...
module.exports = { getAllFeed, getAllSensor, getAllButton, getFeed, updateStatus, subscribeFeed, updateAutomation, subcribeAutomation };
